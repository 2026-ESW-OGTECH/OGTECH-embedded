...
| 안전 | **CO 경보 판정** | 물리 출력 없음 | 35 ppm 3분 지속 WARN · 100 ppm 즉시 ALARM · 30 ppm 미만 30초 해제. 경보음은 텔레메트리를 받은 Jetson 스피커가 낸다(2026-08-31 부저 PB0 제거) |
...
└─ Src/
   ...
   ├─ co_alarm.c           CO 경보 상태기(latched) · 물리 출력 없음
...
다시 생성하고 나머지 모듈은 그대로 이식합니다. 모듈이 스스로 결정하는 핀은 PC9(gate) 하나뿐이며
`JetsonGate_Init()`이 설정합니다.
...
| `co_alarm` | 35/100/30 ppm 경보 상태기(센서 단절 시 latched 유지), 예열·신선도 판정. 소리는 내지 않는다 — 경보음은 Jetson 스피커 |
...
| 판정 | 조건 | 출력 |
|---|---|---|
| ALARM | 100 ppm 즉시 (예열 중에도 적용 — 안전 편향) | `alarm:true` 텔레메트리 → Jetson 스피커 경보음 + 음성 안내 |
| WARN | 35 ppm이 3분 지속 | `level:"warning"` 텔레메트리 → Jetson 스피커 주의음 + 음성 안내 |
| 해제 | 30 ppm 미만이 30초 지속 | 스피커 경보 정지 |

Jetson 전원이 게이트로 차단된 상태(`GATE=OFF`)에서도 **판정은** STM32에서 그대로 돌고 경보는
latched로 남습니다. 다만 2026-08-31 부저(PB0) 출력을 제거했으므로 **그 동안 소리는 나지 않습니다** —
Jetson이 다시 켜져 텔레메트리를 받는 순간 스피커가 경보음을 냅니다. 소리까지 보드 단독으로 내려면
부저를 되살려야 합니다. 실장 검증(연속 20회)은 아직 남아 있습니다.
...
| 항목 | 상태 |
|---|---|
| 진동 모터 · 스트로브 출력 | **미실장.** `ALERT TRAIL` 명령의 상태 전이·ACK·watchdog은 구현했으나 물리 출력이 없다. 보드의 경보 물리 출력은 없다 — 경보음은 Jetson 스피커가 낸다 |
...
2. USART1(9600)·USART2(9600)·USART3(115200)·**UART4(115200, PC10/PC11)** 와 DHT11 데이터 핀(PA0)을 설정합니다.
   ...
   PC9(gate)는 모듈이 직접 초기화하므로 CubeMX에서 다른 용도로 잡지 않습니다.
   PB0는 더 이상 쓰지 않습니다(부저 제거) — 다른 용도로 자유롭게 잡아도 됩니다.
...
| CO 임계 판정 | 구현 완료(2026-08-20, 2026-08-31 부저 출력 제거), 호스트 시뮬레이션(100 ppm 즉시·latched·PB0 무출력) 통과, 빌드·실장 `[미검증]` |
